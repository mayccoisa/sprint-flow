...
**2026-06-21** — refino de design: (1) **cores hardcoded → tokens** (`slate/gray` → `foreground`/`muted-foreground`/`muted`/`border`) em 18 arquivos, destravando o **dark mode** (slate interno 119→16; resíduos são intencionais: scrim de modal, barra de ações dark, fallback de dots); (2) `window.confirm()` nativo → `useConfirm` em `AtlassianToolSettings`; (3) `<PageHeader>` adotado em +10 páginas (Squads, Team, Index, ProductModules, ImportClientInitiatives, UsersManagement, SquadMembers, Tools, SeedData, FormsManagement). Pendentes de migração para `PageHeader` (header especial — voltar/ícone-brand/título editável/toolbar): ProductStrategy, SprintSummary, ReleaseDetail, InitiativeDetail, InitiativesOverview, ProductBacklog, Backlog.
